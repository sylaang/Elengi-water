...
export async function GET(request: Request) {
  ...
  try {
    ...
    const dailySummaries = Array.from({ length: 7 }, (_, i) => {
      ...
      const dayOperations = operations.filter((op: { date: Date }) => 
    ...
    const exportableOperations = operations.map((op: { date: Date; description?: string; amount: number; type: string; user?: { name?: string }; category?: { name?: string } }) => ({
